...
// Test configuration
const TEST_CONTROLLER_URL =
  process.env.TEST_CONTROLLER_URL || "http://192.168.1.100";
...
describe.skip("API Integration Tests", () => {
    ...
    it("should have scenes with correct structure", async () => {
      ...
      if (response.data.scenes.length > 0) {
        ...
        if (scene.transition) {
          ...
          if (scene.transition.t)
            expect(isInteger(scene.transition.t)).toBe(true);
          if (scene.transition.d !== undefined)
            expect(isInteger(scene.transition.d)).toBe(true);
        }
      }
    });
  ...
  describe("POST /data - Update Application Data", () => {
    it("should create a new preset with quoted ID", async () => {
      ...
      const dataResponse = await apiRequest("/data");
      const createdPreset = dataResponse.data.presets.find(
        (p) => p.id === testPresetId,
      );
    ...
    it("should update existing preset field", async () => {
      ...
      // Verify update
      const dataResponse = await apiRequest("/data");
      const updatedPreset = dataResponse.data.presets.find(
        (p) => p.id === testPresetId,
      );
    ...
    it("should create a group with controller_ids array", async () => {
      ...
      // Verify creation
      const dataResponse = await apiRequest("/data");
      const createdGroup = dataResponse.data.groups.find(
        (g) => g.id === testGroupId,
      );
    ...
    it("should create a scene with transitions", async () => {
      ...
      // Verify creation
      const dataResponse = await apiRequest("/data");
      const createdScene = dataResponse.data.scenes.find(
        (s) => s.id === testSceneId,
      );
    ...
    it("should delete preset with DELETE command", async () => {
      ...
      // Verify deletion
      const dataResponse = await apiRequest("/data");
      const deletedPreset = dataResponse.data.presets.find(
        (p) => p.id === testPresetId,
      );
