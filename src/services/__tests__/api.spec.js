...
const { requestTimeout, retryDelay } =
  await import("../../stores/storeConstants.js");
...
describe("ApiService", () => {
    ...
    it("should split large /data POST payloads into throttled chunks", async () => {
      ...
      for (const call of mockFetch.mock.calls) {
        ...
        expect(bodyBytes).toBeLessThanOrEqual(
          apiService._chunking.maxPayloadBytes,
        );
      }
  ...
  describe("Type Safety - HSV Float vs Integer", () => {
    ...
    it("should send preset with integer HSV values to /data endpoint", async () => {
      ...
      expect(Number.isInteger(sentBody['presets[id="test"]'].color.hsv.h)).toBe(
        true,
      );
    });
